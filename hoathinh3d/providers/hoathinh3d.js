...
// src/hoathinh3d/constants.js
var SITE_BASE = "https://hoathinh3d.ee";
...
function findSitePost(titles, season, seasonName) {
  return __async(this, null, function* () {
    ...
    for (const candidate of candidates) {
      ...
      seen.add(slug);
      try {
        const tags = yield fetchJson(`${SITE_BASE}/wp-json/wp/v2/tags?slug=${encodeURIComponent(slug)}`);
        if (!Array.isArray(tags) || tags.length === 0)
          continue;
        const posts = yield fetchJson(`${SITE_BASE}/wp-json/wp/v2/posts?tags=${tags[0].id}&per_page=5`);
